...
import type { ExplorerLog, LogsRepo } from './modules/logs-repo'
import LogViewer from './ui/components/LogViewer'
import LogFilters from './ui/components/LogFilters'
...
function App() {
  const [logsRepo] = useState<LogsRepo>(new EmptyLogsRepo())
  ...
  const filteredLogs = useMemo(() => {
    ...
    if (filters.method) {
      filtered = filtered.filter(log => String(log.requestReceived.method ?? '') === filters.method)
    }

    if (filters.path) {
      filtered = filtered.filter(log =>
        String(log.requestReceived.path ?? '').toLowerCase().includes(filters.path.toLowerCase())
      )
    }

    if (filters.statusCode) {
      filtered = filtered.filter(log => log.responseSent.statusCode === parseInt(filters.statusCode))
    }

    if (filters.protocol) {
      filtered = filtered.filter(log => {
        const headers = log.requestReceived.headers as Record<string, unknown> | undefined
        const upgrade = String(headers?.Upgrade ?? headers?.upgrade ?? '')
        const protocol = upgrade.toLowerCase() === 'websocket' ? 'WS' : 'HTTP'
        return protocol === filters.protocol
      })
    }
